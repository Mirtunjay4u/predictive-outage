-- Create a view for derived event intelligence fields
-- These are computed on-the-fly from existing data for consistency

CREATE OR REPLACE VIEW public.events_intelligence AS
SELECT 
  s.*,
  ...
  CASE
    ...
    WHEN s.etr_confidence = 'HIGH' THEN 'MEDIUM' -- HIGH confidence but band > 2 hours
    ELSE NULL
