      ...
      <header>
        <div className={cn('flex items-start justify-between gap-4 rounded-xl border border-border/60 bg-card shadow-sm', boardroomMode ? 'px-5 py-4' : 'px-4 py-3')}>
          <div>
            ...
            <p className={cn('mt-1 text-muted-foreground', boardroomMode ? 'text-sm' : 'text-xs')}>Event: <span className="font-medium text-foreground">{getExtremeHazard(selectedHazardKey).label} Event</span> • Hazard: <span className="font-medium text-foreground">{getExtremeHazard(selectedHazardKey).label}</span></p>
