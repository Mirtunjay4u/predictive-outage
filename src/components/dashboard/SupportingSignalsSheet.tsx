        ...
        <div ref={contentRef} className="h-[calc(100vh-7rem)] space-y-5 overflow-y-auto py-4 text-sm">
          <section className="space-y-2">
            ...
            <div className="flex flex-wrap items-center gap-2 text-xs">
              ...
              <Badge variant="outline" className={sourceLabel.toLowerCase().includes('nemotron') ? 'border-[hsl(80,100%,36%)]/40 bg-[hsl(80,100%,36%)]/10 text-[hsl(80,100%,36%)]' : ''}>Provenance: {sourceLabel}</Badge>
              <span className={DASHBOARD_TIMESTAMP_CLASS}>Updated {formatDashboardTime(timestamp)}</span>
              <Button variant="outline" size="sm" onClick={() => void copySummary()} className={cn('h-7 text-[11px] transition-colors hover:border-[hsl(80,100%,36%)]/50 hover:bg-[hsl(80,100%,36%)]/15 hover:text-[hsl(80,100%,36%)]', DASHBOARD_INTERACTIVE_BUTTON_CLASS)}><Copy className="mr-1.5 h-3 w-3" />Copy summary</Button>
            </div>
          </section>
