    ...
    <div className="rounded-lg border border-border/50 bg-card px-4 py-3 space-y-3">
      ...
      {policyStatus === 'done' && gate && (
        ...
        >
          ...
          {/* Engine ETR Band */}
          {policy?.etrBand?.band && (
            <div className="flex flex-col gap-1 rounded-lg border border-border/50 bg-muted/20 px-3 py-2">
              <div className="flex items-center gap-2">
                ...
                <span
                  className={cn(
                    ...
                    policy.etrBand.band === 'HIGH'
                      ? 'bg-emerald-500/10 border-emerald-400/30 text-emerald-700 dark:text-emerald-300'
                      : policy.etrBand.band === 'MEDIUM'
                      ? 'bg-amber-500/10 border-amber-400/30 text-amber-700 dark:text-amber-300'
                      : policy.etrBand.band === 'LOW'
                ...
                {policy.etrBand.confidence != null && (
                  <span className="ml-auto text-[10px] font-semibold tabular-nums text-muted-foreground">
                    {Math.round(policy.etrBand.confidence * 100)}% confidence
                  </span>
                )}
              </div>
              {(policy.etrBand.rationale ?? []).length > 0 && (
                <p className="text-[10px] text-muted-foreground leading-snug">
                  {policy.etrBand.rationale![0]}
                </p>
              )}
            </div>
          )}
